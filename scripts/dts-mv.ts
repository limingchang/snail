...
/** 以根目录为基础解析路径 */
const fromRoot = (...paths: string[]) => join(rootDir, '..', ...paths);
...
async function resolve(pkgName: string) {
  try {
    ...
    const cpTasks = sourceFiles.map((file) => {
      ...
      console.log(chalk.bold.hex('#fc7930')(`[${pkgName}] moving\n`),chalk.hex("#9abeaf")(`${source} => ${target}`));
    ...
    console.log(chalk.green(`[${pkgName}]: moved successfully!`));  
  } catch (e) {
    console.log(chalk.red(`[${pkgName}]: failed to move!`));
  }
}
