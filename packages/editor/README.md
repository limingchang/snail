...
| 用途 | 编写模板 | 填写模板并打印 |
| 文档 | 可编辑 | 只读 |
| 工具栏 | 显示 | 隐藏，只在模板没被 `template` 定死时显示「模板」一个分区 |
| 变量 | 显示为标签徽章 | 显示为变量值 |
| 操作 | 保存 | 填写 + 打印 |
...
设计模式下这些控件住在工具栏的**「模板」分区**里（第一个分区，在「格式」之前）：拉取/刷新列表、
选中一条再点它的「载入」，或者用「新建模板」从空白文档开始。填写模式下这个分区换成「加载本地
模板」——把本机保存下来的那份模板载回来；`template` 是 `kind: "remote"`（模板已被定死）时整个
工具栏都不显示。
...
两种方式里，**本地字符**（`{ kind: "local" }`，或只给 `onSave`）是不需要后端的那一种：模板字符
写进浏览器自己的存储，刷新之后还在。填写模式的「模板」分区里的「加载本地模板」读的就是同一个
槽位（`save` 是本地目标时用它的 `storageKey`，否则用默认键 `snail-editor-template`），所以
「设计 → 存本地 → 打开填写 → 载回来」是一条闭环路径。
...
`tools` 一道闸门。它是第一个分区（在「格式」之前），填写模式下也留着它——除非 `template` 已经把
模板本身定死（`kind: "remote"`），那时没有可选项，工具栏整个不显示。

`DEFAULT_TOOLS` 的两头因此是固定的：第一项 `template`，最后一项 `watermark`（水印是成品合同的
一部分，放在最后是因为它是作者最后才会去动的东西）。`variable`、`qrcode`、`print` 仍需点名。

| 分区 | 需要的扩展 |
| --- | --- |
| 模板 `template` | —（模板管线，不受扩展闸门约束） |
| 格式 `font` | `textStyle` |
...
- **页眉页脚是每一页的。** 改任意一页的任一格之后，同步插件会把那条栏的每一格复制到同侧其他页；
  放着页码或 Logo 的格子不可编辑，双击会弹提示说明原因。逻辑都在 `extensions/page/utils/furniture*.ts`。
- **输入法组合期间不改文档。** 分页器与区域规范化插件都带 `compositionstart`/`compositionend` 闸门，
  否则多字输入会被打断成拼音。
- **表格列宽拖动与分页互斥。** 分页在拖动期间让路，松手后兜底清理未收尾的拖动状态
  （`extensions/table/resizeGuard.ts`），否则列宽只能调一次且光标会卡在宽度调整上。
- **水印改设置一定重画。** widget 的 key 里编进了设置指纹（`watermarkSettingsKey`），不然
  ProseMirror 会沿用旧 DOM，看起来就是「只有第一次设置生效」。
- **填写模式下变量就是正文**：继承所在段落的颜色、字号、行高，没有底色与内边距。
